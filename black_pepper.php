<?php $page='products';
$title='Black & White Pepper :: Nani Agro Foods Pvt. Ltd.';
	    include "include/header.php";
	 ?>

    <!-- Baner Section  -->
    <div class="baner-section-wrapper">
      <div class="container">
        <div class="black-baner-inner-wraper">
        <img class="banerimage" src="./assets/images/blackbanner.jpg" alt="">
          <div class="bottom-wrapper">
            <h1>Black & White Pepper</h1>
            <div class="bottom-inner-wrapper">
              <div class="bottom-left">
                <p>
                    We pre-process all types of black pepper & white pepper which shall be exported in whole and ground form as per customer requirements. We have pre-processing activities like grading, spiraling, destoning, washing and sun-drying based on the requirement from our customer.
                </p>
              </div>
              <div class="bottom-right">
                <a href="#blackenquire"><img src="./assets/images/pepper-enquire-now.png" alt=""></a>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
    <!-- Baner Section  -->

    <!-- Turmeric Process  -->
    <div class="process-wrapper">
      <div class="container">
        <div class="process-inner">
          <h3 class="process-title blackpepper-process-title">Black & White Pepper Process</h3>
          <div class="process-block-wrapper">
            <!-- Process Block  -->
            <div class="process-block">
              <div class="process-block-left">
                <img src="./assets/images/Manual-Pre-Cleaning.png" alt="" />
              </div>
              <div class="process-block-right">
                <h5>Pre Cleaning</h5>
              </div>
            </div>
            <!-- Process Block  -->
            <!-- Process Block  -->
            <div class="process-block">
              <div class="process-block-left">
                <img src="./assets/images/De-Stoning1.png" alt="" />
              </div>
              <div class="process-block-right">
                <h5>De-Stoning</h5>
              </div>
            </div>
            <!-- Process Block  -->
             <!-- Process Block  -->
             <div class="process-block">
              <div class="process-block-left">
                <img src="./assets/images/Packing.png" alt="" />
              </div>
              <div class="process-block-right">
                <h5>Packing</h5>
              </div>
            </div>
            <!-- Process Block  -->
            <!-- Process Block  -->
            <div class="process-block">
              <div class="process-block-left">
                <img src="./assets/images/Spiralling.png" alt="" />
              </div>
              <div class="process-block-right">
                <h5>Spiralling</h5>
              </div>
            </div>
            <!-- Process Block  -->
            <!-- Process Block  -->
            <div class="process-block">
              <div class="process-block-left">
                <img
                  src="./assets/images/Grinding-Pre-Cleaned-Turmeric.png"
                  alt=""
                />
              </div>
              <div class="process-block-right">
                <h5>
                  Grding
                </h5>
              </div>
            </div>
            <!-- Process Block  -->
          </div>
        </div>
      </div>
    </div>
    <!-- Turmeric Process  -->

      <!-- Product Range  -->
      <div class="product-range-wrapper">
        <div class="container">
          <div class="product-range">
            <h2 class="black-range-title">Black & White Pepper Product Range</h2>
            <div class="product-range-inner">
                <div class="product-range-left">
                    <ul>
                        <li>Black Pepper - Indian Origin - 500 GL , 550 GL and 600 GL Cleaned and Washed</li>
                        <li>Black Pepper - Sri Lankan Origin - 550 GL Cleaned and Washed</li>
                        <li>White Pepper - Indian Origin - 620 GL Cleaned and Washed - Bleached and Unbleached</li>
                    </ul>
                </div>
            </div>
          </div>
        </div>
    </div>
    <!-- Product Range  -->

    <!-- Private Label  -->
    <div class="private-label-wrapper">
      <div class="container">
        <div class="private-label">
          <div class="private-lebel-inner-wrapper">
            <div class="private-label-inner-left">
              <h2 class="label-title">Private Label Packaging</h2>
              <img src="./assets/images/product.png" alt="" />
              <p>
              From designing to manufacturing and labelling, our team will provide you with end-to-end solutions, catering to all your packaging requirements - be it formatting, choosing the appropriate size and shape, designing, labelling and barcoding.
              </p>
            </div>
            <div class="private-label-inner-right">
              <h2 class="specification-title">
                Specification & Packaging Options
              </h2>
              <!-- Specification Box  -->
              <div class="specification-wrapper">
                <div class="specification-left specification-yellow">
                  <p>Origin</p>
                </div>
                <div class="specification-right specification-yellow">
                  <p>India, Sri Lanka</p>
                </div>
              </div>
              <!-- Specification Box  -->           
               <!-- Specification Box  -->
               <div class="specification-wrapper">
                <div class="specification-left specification-yellow">
                  <p class="black-capacity">Capacity</p>
                </div>
                <div class="specification-right specification-yellow">
                  <p>Black/ White Pepper Whole - 25 MT/day</p>
                </div>
              </div>
              <!-- Specification Box  -->
              <!-- Specification Box  -->
              <div class="specification-wrapper pack-margin">
                <div class="specification-left specification-white pack">
                  <p>Pack Size</p>
                </div>
                <div class="specification-right specification-white">
                  <p>
                    10g, 20g, 50g, 100g, 200g,<br />500g, 1kg, 5kg, 10kg, 25kg
                  </p>
                </div>
              </div>
              <!-- Specification Box  --> 
               <!-- Specification Box  -->
               <div class="specification-wrapper">
                <div class="specification-left specification-white packaging">
                    <p>Packaging<br/>Varieties</p>
                </div>
                <div class="specification-right specification-white">
                    <p>Mono Carton Boxes</p>
                    <p>Pouches</p>
                    <p>Zip lock Pouches</p>
                    <p>Plastic Pet Jar & Tin</p>
                </div>
            </div>
            <!-- Specification Box  -->          
            </div>
          </div>
        </div>
      </div>
    </div>
    <!-- Private Label  -->

      <!-- Harvest Calendar  -->
      <div class="harvest-calender-wrapper">
        <div class="container">
            <div class="harvest-inner-wrapper">
                <h2 class="harvest-title">Harvest Calendar</h2>
                <div class="month-wrapper">
                    <!-- Month  -->
                    <div class="month month-yellow">
                        <div class="month-name">
                            <h2>Jan</h2>
                        </div>
                    </div>
                    <!-- Month  -->
                     <!-- Month  -->
                     <div class="month month-red">
                        <div class="month-name">
                            <h2>Feb</h2>
                        </div>
                    </div>
                    <!-- Month  -->
                     <!-- Month  -->
                     <div class="month month-red">
                        <div class="month-name">
                            <h2>Mar</h2>
                        </div>
                    </div>
                    <!-- Month  -->
                     <!-- Month  -->
                     <div class="month month-yellow">
                        <div class="month-name">
                            <h2>Apr</h2>
                        </div>
                    </div>
                    <!-- Month  -->
                     <!-- Month  -->
                     <div class="month month-yellow">
                        <div class="month-name">
                            <h2>May</h2>
                        </div>
                    </div>
                    <!-- Month  -->
                     <!-- Month  -->
                     <div class="month month-yellow">
                        <div class="month-name">
                            <h2>Jun</h2>
                        </div>
                    </div>
                    <!-- Month  -->
                     <!-- Month  -->
                     <div class="month month-yellow">
                        <div class="month-name">
                            <h2>Jul</h2>
                        </div>
                    </div>
                    <!-- Month  -->
                     <!-- Month  -->
                     <div class="month month-yellow">
                        <div class="month-name">
                            <h2>Aug</h2>
                        </div>
                    </div>
                    <!-- Month  -->
                     <!-- Month  -->
                     <div class="month month-yellow">
                        <div class="month-name">
                            <h2>Sep</h2>
                        </div>
                    </div>
                    <!-- Month  -->
                     <!-- Month  -->
                     <div class="month month-yellow">
                        <div class="month-name">
                            <h2>Oct</h2>
                        </div>
                    </div>
                    <!-- Month  -->
                     <!-- Month  -->
                     <div class="month month-red">
                        <div class="month-name">
                            <h2>Nov</h2>
                        </div>
                    </div>
                    <!-- Month  -->
                     <!-- Month  -->
                     <div class="month month-red">
                        <div class="month-name">
                            <h2>Dec</h2>
                        </div>
                    </div>
                    <!-- Month  -->
                </div>
                <div class="harvest-bottom">
                    <div class="harvest-bottom-left">
                        <p>Black & White Pepper is generally harvested during the month of February, March, November and December. Looking forward to import pepper? Plan your delivery here.</p>
                    </div>
                    <div class="harvest-bottom-right">
                        <a href="#"><img src="./assets/images/pepper-VIEW-MORE.png" alt=""></a>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <!-- Harvest Calendar  -->

    <!-- Language  -->
    <div class="language-wrapper">
      <div class="container">
        <div class="languageinner">
          <h2 class="language-title black-language-title">Black & White Pepper In other Languages</h2>
          <div class="language-inner-wrapper blackpepper-inner-wrapper">
            <div class="language">
              <h4>English: <span>Black Pepper & White Pepper</span></h4>
            </div>
            <div class="language">
              <h4>German: <span>Schwarzer Pfeffer & Weißer Pfeffer</span></h4>
            </div>
            <div class="language">
              <h4>French: <span>Poivre Noir & Poivre Blanc</span></h4>
            </div>
            <div class="language">
              <h4>Italian: <span>Pepe Nero & Pepe Bianco</span></h4>
            </div>
            <div class="language">
              <h4>Spanish: <span>Pimienta Negra y Pimienta Blanca</span></h4>
            </div>
            <div class="language">
              <h4>Swedish: <span>Svartpeppar och vitpeppar</span></h4>
            </div>
            <div class="language">
              <h4>Russian: <span>Черный перец и белый перец</span></h4>
            </div>
            <div class="language">
              <h4>Portuguese: <span>Pimenta preta e pimenta branca</span></h4>
            </div>
            <div class="language">
              <h4>Hindi: <span>काली मिर्च और सफेद मिर्च</span></h4>
            </div>
            <div class="language">
              <h4>Mandarin Chinese: <span>黑胡椒和白胡椒</span></h4>
            </div>
          </div>
        </div>
      </div>
    </div>
    <!-- Language  -->

    <!-- Enquire Now Wrapper  -->
    <div class="enquire-wrapper" id="blackenquire">
        <div class="container">
          <div class="enquire">
            <h2 class="enquire-title">Enquire Now</h2>
            <div class="enquire-inner-wrapper">
                <div class="enquire-inner-left">
                    <form action="" method="post">
                        <div class="row">
                            <div class="col">
                              <input type="text" class="form-control" placeholder="NAME">
                            </div>
                            <div class="col">
                              <input type="text" class="form-control" placeholder="E-MAIL">
                            </div>
                          </div>
                          <div class="row">
                            <div class="col">
                              <input type="tel" class="form-control" placeholder="PHONE">
                            </div>
                            <div class="col">
                              <input type="text" class="form-control" placeholder="COUNTRY">
                            </div>
                          </div>
                          <div class="form-group">
                            <textarea class="form-control"      placeholder="MESSAGE" id="exampleFormControlTextarea1" rows="6"></textarea>
                          </div>

                          <div class="submit-btn-wrapper">
                              <button type="submit" class="submit-btn">Submit Details</button>
                          </div>
                        
                    </form>
                </div>
                <div class="enquire-inner-right">
                    <div class="schedule-inner-right">
                        <div class="schedule-mobile">
                            <img src="./assets/images/mobile-image.jpg" alt="" />
                            <h3>Schedule<br />Your<br />Discovery<br />Call</h3>
                             <a href="https://calendly.com/naniagrofood/nani-agro-food" target="_blank">Schedule now</a>
                        </div>
                    </div>
                </div>
            </div>
          </div>
        </div>
    </div>
    <!-- Enquire Now Wrapper  -->

    <?php 
    include "include/footer.php";
  	?> 